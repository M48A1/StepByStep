...
if command -v ufw >/dev/null; then
    ufw --force enable
    ufw allow "${PORT}"/tcp
    echo -e "${GREEN}UFW 已启用并放行端口 ${PORT}${NC}"
fi
...
VLESS_LINK="vless://${UUID}@${SERVER_IP}:${PORT}?security=reality&encryption=none&pbk=${PUBLIC_KEY}&fp=chrome&type=tcp&sni=${CUSTOM_SNI}&sid=${SHORT_ID}#${NODE_NAME}"
